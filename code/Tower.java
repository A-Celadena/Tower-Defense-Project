...
public abstract class Tower {
    ...
    public Enemy getClosestEnemy(ArrayList<Enemy> enemies) {
        ...
        for (Enemy e : enemies) {
            if (calculateDistance(e) <= minDistance) {
                closestTarget = e;
                minDistance = calculateDistance(e);
            }
        }
    ...
    public double calculateDistance(Enemy e) {
        ...
        double distance = Math.sqrt( (Math.pow(e.getX() - this.x,2)) + (Math.pow(e.getY() - this.y, 2)) );
        
        return distance;  // placeholder — replace this
    }
