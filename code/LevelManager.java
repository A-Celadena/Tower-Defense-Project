...
public class LevelManager {
    ...
    public void addTower(int x, int y, int type) {
        ...
        if (type == 0) {
            BasicTower bas = new BasicTower (x, y);
            if (gold >= bas.getCost()) {
                gold -= bas.getCost();
                activeTowers.add(bas);
            }
        } else if (type == 1) {
            RapidTower rap = new RapidTower (x, y);
            if (gold >= rap.getCost()) {
                gold -= rap.getCost();
                activeTowers.add(rap);
            }
        } else {
            SniperTower snipe = new SniperTower (x, y);
            if (gold >= snipe.getCost()) {
                gold -= snipe.getCost();
                activeTowers.add(snipe);
            }
        }
    ...
    private void spawnEnemy() {
        ...
        if (getWaveNumber() >= 3 && enemiesSpawnedThisWave == enemiesPerWave - 1) {
            Tank tan = new Tank(path);
            activeEnemies.add(tan);
        } else if (getWaveNumber() >= 2 && enemiesSpawnedThisWave % 3== 2) {
            Speedster speed = new Speedster(path);
            activeEnemies.add(speed);
        } else {
            Grunt grun = new Grunt(path);
            activeEnemies.add(grun);
        }
